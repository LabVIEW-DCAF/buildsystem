def call(path, lvVersion) {
        ...
        def vipb_json = JsonOutput.toJson(['VIPB Path': "${path}","Install after build": false, "LabVIEW Version":"${lvVersion}", Executor_Number: env.EXECUTOR_NUMBER.toString(), Workspace_Path : env.WORKSPACE+'\\build_temp'])
        echo vipb_json
